...
----1.employees жадвалида “100-Steven King” каби чиқсин: emp_id + '-' + first_name + ' ' + last_name.
...
SELECT CONCAT(EMPLOYEE_ID, '-', FIRST_NAME, ' ', LAST_NAME) AS EmpLine
FROM Employees
WHERE EMPLOYEE_ID = 100;   -- ихтиёрий фильтр


----2.employees жадвалида телефон рақамларидаги '124' бўлакчасини '999' га алмаштириб янгилагинг.


UPDATE Employees
SET PHONE_NUMBER = REPLACE(PHONE_NUMBER, '124', '999');

----3.Исми A, J ёки M ҳарфлари билан бошланадиган барча ходимлар учун first_name ва унинг узунлигини чиқаринг. Устунларга мос номлар беринг, натижани исм бўйича сортланг.
...
----4.Ҳар бир manager_id бўйича умумий ойликни топинг (Employees).
...
----5.TestMax жадвалида ҳар қатор учун Max1, Max2, Max3 устунларидан энг катта қийматни ва шу қаторнинг Year ни чиқаринг.

SELECT 
  Year1,
  (SELECT MAX(v) 
   FROM (VALUES (Max1),(Max2),(Max3)) AS t(v)) AS MaxOf3
FROM TestMax;



----6.cinema’да ID’си тақ (odd) бўлган ва тавсифи “boring” эмас бўлган фильмларни топинг.
...
----7.SingleOrder жадвалида Id бўйича сортланг, лекин Id=0 қатори ҳамиша энг охирида турсин. Битта ORDER BY ифодасида бажаринг.

SELECT *
FROM SingleOrder
ORDER BY IIF(Id = 0, 2147483647, Id);


----8.person жадвалида бир нечта устунлар орасидан биринчи NULL эмас қийматни танлаб олинг (ҳаммаси NULL бўлса, NULL қайтсин).
...
----Ўртача қийин топшириқлар----

----1.Students жадвалида FullName устунини 3 қисмга бўлинг: Firstname, Middlename, Lastname.
...
----2.Калифорнияга (CA) етказиб берилиши бўлган ҳар бир мижоз учун, ўша мижознинг Техасга (TX) етказилган барча буюртмалари рўйхатини беринг (Orders).
...
----3.DMLTable бўйича қийматларни гуруҳлаб бир сатрига бириктиринг (group-concat).

SELECT STRING_AGG(String, ' ') WITHIN GROUP (ORDER BY SequenceNumber) AS QueryText
FROM DMLTable;


----4.Исми+фамилияси бирга ёзилганда “a” ҳарфи камида 3 марта учрайдиган ходимларни топинг.

WITH E AS (
  SELECT *, LOWER(FIRST_NAME + LAST_NAME) AS full_lower
  FROM Employees
)
SELECT EMPLOYEE_ID, FIRST_NAME, LAST_NAME
FROM E
WHERE (LEN(full_lower) - LEN(REPLACE(full_lower, 'a', ''))) >= 3;


----5.Ҳар бир бўлимдаги ходимлар сони ва шуларнинг нечта фоизи 3 йилдан ортиқ иш стажига эга (Employees).
...
-----1.Students жадвалида ҳар қатор қиймати ўзигача бўлган олдинги қаторлар қиймати билан йиғиндига алмаштирилсин (кўчма йиғинди).


WITH RT AS (
  SELECT 
    StudentID,
    SUM(Grade) OVER (ORDER BY StudentID ROWS UNBOUNDED PRECEDING) AS run_total
  FROM Students
)
UPDATE s
SET Grade = r.run_total
FROM Students AS s
JOIN RT AS r ON r.StudentID = s.StudentID;
...
SELECT 
  ...
  FullName,
  Grade,
  SUM(Grade) OVER (ORDER BY StudentID ROWS UNBOUNDED PRECEDING) AS RunningTotal
FROM Students;


-----2.Бир хил туғилган кунга эга (бир хил ой-кун) бўлган талабаларни топинг (Student).
...
SELECT s.*
FROM Student AS s
JOIN Dups AS d ON d.Birthday = s.Birthday
ORDER BY s.Birthday, s.StudentName;


-----3.PlayerScores’да жуфтлик (A,B) бир неча марта учраса, уларнинг баллари йиғилиб, ҳар бир ноёб жуфтлик учун бир қатор қайтсин.
...
-----4.Берилган сатр 'tf56sd#%OqH' таркибидан: катта ҳарфлар, кичик ҳарфлар, рақамлар, бошқа белгиларни алоҳида устунларга ажратиб чиқаринг
...
Chars AS (
  SELECT SUBSTRING(@s, n, 1) AS ch FROM Tally
),
Classed AS (
  SELECT ch,
         CASE 
...
SELECT
  STRING_AGG(CASE WHEN cls='UPPER' THEN ch END, '') AS UppercaseLetters,
  STRING_AGG(CASE WHEN cls='LOWER' THEN ch END, '') AS LowercaseLetters,
  STRING_AGG(CASE WHEN cls='DIGIT' THEN ch END, '') AS Digits,
  STRING_AGG(CASE WHEN cls='OTHER' THEN ch END, '') AS Others
FROM Classed
