...
-----1.	(TestMultipleColumns) Name устунини вергул (,) бўйича икки алоҳида устунга ажратинг: Name ва Surname.

SELECT 
    Id,
    LTRIM(RTRIM(LEFT([Name], CHARINDEX(',', [Name] + ',') - 1)))  AS [Name],
    LTRIM(RTRIM(SUBSTRING([Name],
                          CHARINDEX(',', [Name] + ',') + 1,
                          LEN([Name]))))                            AS [Surname]
FROM TestMultipleColumns;


----2.	(TestPercent) Қаторнинг ўзида % белгиси мавжуд бўлган сатрларни топинг.

SELECT *
FROM TestPercent
WHERE Strs LIKE '%[%]%';   -- '['%' ']' — лексикал тарзда '%'


----3.	(Splitter) Сатрни нуқта (.) бўйича бўлиб чиқаринг.

SELECT s.Id, x.[ordinal] AS PartNo, x.[value] AS PartValue
FROM Splitter AS s
CROSS APPLY STRING_SPLIT(s.Vals, '.', 1) AS x;   -- 1 => ordinal қайтарилади




----4.	(testDots) Vals устуни қийматида икки тадан кўп нуқта (.) бор бўлган барча қаторларни кайтаринг.
...
-----5.	(CountSpaces) Берилган сатрдаги бўш жойлар (пробеллар) сонини ҳисобланг.
...
-----6.	(Employee) Ойлиги ўз менежериникидан кўпроқ бўлган ходимларни топинг.
...
-----7.	(Employees) Компанияда 10 йилдан кўп, лекин 15 йилдан кам ишлаган ходимларни топинг. Employee ID, First Name, Last Name, Hire Date ва хизмат йиллари (жорий сана билан hire_date орасидаги йиллар) ни чиқаринг.
...
-----1.	(weather) Кечкиндги (кечаги) кунига нисбатан ҳарорати юқорироқ бўлган саналарнинг Id ларини топинг.
...
-----2.	(Activity) Ҳар бир ўйинчи учун биринчи кириш (логин) санасини ҳисобот қилинг.
...
-----3.	(fruits) Рўйхатдан учинчи элементни қайтаринг.
...
-----4.	(Employees) HIRE_DATE асосида ҳар бир ходимнинг “Ишлаб юриш босқичи” ни аниқланг:
-----o	1 йилдан кам → New Hire
-----o	1–5 йил → Junior
-----o	5–10 йил → Mid-Level
-----o	10–20 йил → Senior
-----o	20 йилдан кўп → Veteran
...
-----5.	(GetIntegers) Vals устунидаги сатр бошида келадиган бутун сонни ажратиб олинг.
...
SELECT 
  ...
  TRY_CONVERT(int,
      CASE 
        WHEN VALS LIKE '[0-9]%' 
          THEN LEFT(VALS, NULLIF(PATINDEX('%[^0-9]%', VALS), 0) - 1)
        ELSE NULL
      END
  ) AS LeadingInteger
FROM GetIntegers;
...
-----1.	(MultipleVals) Вергул билан ажратилган сатрдаги биринчи икки ҳарфнинг ўрнига-ўрин алмаштиринг.

SELECT mv.Id,
       mv.Vals,
       CONCAT(SecondTok, ',', FirstTok, RestAfterSecond) AS Swapped
FROM MultipleVals AS mv
CROSS APPLY (
  SELECT CHARINDEX(',', mv.Vals) AS p1
) A
CROSS APPLY (
  SELECT 
    LEFT(mv.Vals, p1-1)                              AS FirstTok,
    STUFF(mv.Vals, 1, p1, '')                        AS Tail1
) B
CROSS APPLY (
  SELECT CHARINDEX(',', Tail1) AS p2
) C
CROSS APPLY (
  SELECT 
    LEFT(Tail1, p2-1)                                 AS SecondTok,
    STUFF(Tail1, 1, p2, '')                           AS RestAfterSecond
) D;

----2.	(sdgfhsdgfhs@121313131) Берилган сатрнинг ҳар бир белгисини алоҳида қаторга айлантирадиган жадвал қайтаринг (яъни ҳар бир белги — алоҳида ряд).
...
INTO dbo.StringChars;           -- янги жадвал яратилади
...
-----3.	(Activity) Ҳар бир ўйинчи бўйича энг биринчи логин қилинган қурилмани ҳисобот қилинг.
...
-----4.	(rtcfvty34redt) Сатрдаги бутун сон қийматларини ва ҳарфли қийматларни иккита алоҳида устунга ажратиб чиқаринг.
...
Classed AS (
  SELECT ch,
         CASE WHEN ch LIKE '[0-9]' THEN 'DIGIT' ELSE 'LETTER' END AS cls
  FROM Chars
)
SELECT
  STRING_AGG(CASE WHEN cls='LETTER' THEN ch END, '') AS Letters,
